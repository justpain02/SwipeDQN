...
class SBBEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-500, high=500,
                                            shape=(2 + BALL_NUM_GOAL,), dtype=np.float64)
    def log_begin(self):
        print("log_begin")
        self.Data = {}
    ...
    def action_Prepare(self):
        ...
        while True:
            ...
            if self.Game['Map'][cursor][0] == None:
                self.Game['Map'][cursor][0] = Block('block', self.score)
                break

        left_ball = self.score // 20 + 1
        for x in range(self.Game['width']):
            if left_ball == 0:
                break
            ...
            if block == None:
                if random.randint(0, 1) == 1:
                    self.Game['Map'][x][0] = Block('block', self.score)
                    left_ball -= 1
    ...
    def step(self, action):
        ...
        #
        #     pygame.display.flip()
        # action : 0 -> 왼쪽 rad , 1 -> 오른쪽 rad, 2 -> 발사
        if self.Game['State'] == 'shoot' and (not self.user):
            if action == 0:
                ...
                if -pi + 0.15 < self.rad < 0 - 0.15:
                    deg = (self.rad + pi - 0.15) / (pi - 0.3)
                    self.action_Shoot(deg)


        if self.Game['State'] == 'shooting':
            print("shooting_start")
            if self.Game['Left Balls'] > 0:
        ...
        for ball in balls:
            ...
            # ============================
        self.t += 1
        observation = [self.rad, self.score] + list(self.prev_actions)
        ...
        if self.Game['State'] == 'lose':
            ...
            self.done = False
        info = {}
        return observation, self.score, self.done, info
    def initialize(self):
        print("initialize")
    ...
    def reset(self):
        ...
        self.initialize()
        self.action_space = spaces.Discrete(3)
        self.prev_actions = deque(maxlen=BALL_NUM_GOAL)
        for i in range(BALL_NUM_GOAL):
            self.prev_actions.append(-1)
        observation = [self.rad, self.score] + list(self.prev_actions)
        observation = np.array(observation)
        self.total_reward = 0  # 0에서 시작
